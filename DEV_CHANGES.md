...
## 2026-03-02（车厢容量改为品质缩放）

### 改动摘要
- 车厢容量从固定 500 改为“基础 200 + 品质倍率自动放大”。
- 目标为传奇品质自动达到 500（按默认 2.5x 倍率）。

### 具体改动
- `AssemblyWagon/prototypes/entities/assembly-wagon.lua`
  - `inventory_size` 从 `500` 调整为 `200`。
  - 新增 `quality_affects_inventory_size = true`。

### 备注
- 该方案为定义阶段一次性生效，不增加运行时脚本循环开销。

## 2026-03-02（创建时继承车厢品质）

### 改动摘要
- 伴生组装机在创建时直接继承车厢品质，确保高/低品质车厢对应同品质组装机。

### 具体改动
- `AssemblyWagon/scripts/builder.lua`
  - 在 `surface.create_entity({...})` 创建 `wagon-assembler` 时新增 `quality = entity.quality`。

### 备注
- 本次仅实现“创建时品质对应”，不包含升级/降级过程中的状态迁移与进度同步逻辑。

## 2026-03-01（英文本地化与中文键集同步）

### 改动摘要
- 将英文本地化文件补齐为与中文本地化一致的键集，避免英文环境下出现缺失文本。
